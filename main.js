...
function displayResults(prediction) {
    ...
    // Sort prediction to find the highest score
    prediction.sort((a, b) => b.probability - a.probability);
    
    const topResult = prediction[0];
    const animalName = topResult.className === 'Class 1' ? '강아지' : '고양이'; // 모델 클래스명 확인 필요 (일반적으로 Class 1, Class 2 등)
    // 참고: 제공된 모델의 클래스명을 정확히 모를 경우를 대비해 실제 모델 결과의 className을 사용하도록 보완 가능
